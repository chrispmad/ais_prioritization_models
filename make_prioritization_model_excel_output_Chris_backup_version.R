...
d_bins = d |> 
  ...
  dplyr::mutate(native_species_in_wb = dplyr::case_when(
    native_species_in_wb <= 3 ~ 1,
    native_species_in_wb <= 6 ~ 2,
    native_species_in_wb > 6 ~ 3,
    T ~ 0
  )) |> 
  ...
  dplyr::mutate(introduction_risk_b = dplyr::case_when(
    introduction_risk_mean <= 0.33 ~ 1,
    introduction_risk_mean <= 0.66 ~ 2,
    introduction_risk_mean > 0.66 ~ 3,
    T ~ 0
  )) |>
  dplyr::mutate(wildlife_habitat_areas_b = dplyr::case_when(
    wildlife_habitat_areas <= 2 ~ 1,
    wildlife_habitat_areas > 2 & wildlife_habitat_areas <= 10 ~ 2,
    wildlife_habitat_areas > 10 ~ 3,
    T ~ 0
  )) |> 
  dplyr::mutate(wildlife_habitat_areas_hectares_b = dplyr::case_when(
    wildlife_habitat_areas_hectares <= 100000 ~ 1,
    wildlife_habitat_areas_hectares > 100000 & wildlife_habitat_areas_hectares <= 300000 ~ 2,
    wildlife_habitat_areas_hectares > 300000 ~ 3,
    T ~ 0
  )) |> 
...
conseq = d_bins |> 
  dplyr::select(Region:Established_in_Waterbody, sara_in_wb:other_ais_in_wb_names, other_ais_in_wb_b:cdc_listed_downstream_b, 
                wildlife_habitat_areas, wildlife_habitat_areas_b,
                wildlife_habitat_areas_hectares, wildlife_habitat_areas_hectares_b)
...
openxlsx::setColWidths(my_wb,"metadata",cols = 2:ncol(metadata), widths = 100)

# Add another sheet to describe the binning levels.
openxlsx::addWorksheet(my_wb, "Binning Levels")

bin_levels_df = data.frame(
  variable = c("number_inflows_b","other_ais_in_wb_b","native_species_in_wb","sara_in_wb_b",
               "sara_downstream_b","COSEWIC_in_wb_b","COSEWIC_downstream_b","cdc_listed_in_wb_b",
               "cdc_listed_downstream_b","maxent_suitability_max_b","maxent_suitability_mean_b",
               "m_suit_uncertainty_b","introduction_risk_b","wildlife_habitat_areas_b","wildlife_habitat_areas_hectares_b"),
  bin_levels = c("number_inflows <= 5 ~ 1, number_inflows <= 25 ~ 2, number_inflows > 25 ~ 3",
                 "other_ais_in_wb <= 3 ~ 1, other_ais_in_wb <= 6 ~ 2, other_ais_in_wb > 6 ~ 3",
                 "native_species_in_wb <= 3 ~ 1, native_species_in_wb <= 6 ~ 2, native_species_in_wb > 6 ~ 3",
                 "sara_in_wb == 0 ~ 1, sara_in_wb < 3 ~ 2, sara_in_wb >= 3 ~ 3",
                 "sara_downstream == 0 ~ 1, sara_downstream < 3 ~ 2, sara_downstream >= 3 ~ 3",
                 "COSEWIC_in_wb == 0 ~ 1, COSEWIC_in_wb < 3 ~ 2, COSEWIC_in_wb >= 3 ~ 3",
                 "COSEWIC_downstream == 0 ~ 1, COSEWIC_downstream < 3 ~ 2, COSEWIC_downstream >= 3 ~ 3",
                 "cdc_listed_in_wb == 0 ~ 1, cdc_listed_in_wb < 3 ~ 2, cdc_listed_in_wb >= 3 ~ 3",
                 "cdc_listed_downstream == 0 ~ 1, cdc_listed_downstream < 3 ~ 2, cdc_listed_downstream >= 3 ~ 3",
                 "wb_maxent_suitability_max <= 0.33 ~ 1, wb_maxent_suitability_max <= 0.66 ~ 2, wb_maxent_suitability_max > 0.66 ~ 3",
                 "wb_maxent_suitability_mean <= 0.33 ~ 1, wb_maxent_suitability_mean <= 0.66 ~ 2, wb_maxent_suitability_mean > 0.66 ~ 3",
                 "wb_maxent_training_AUC >= 0.9 ~ 1, wb_maxent_training_AUC >= 0.8 ~ 2, wb_maxent_training_AUC < 0.8 ~ 3",
                 "introduction_risk_mean <= 0 ~ 1, other_ais_in_wb_b <= 6 ~ 2, other_ais_in_wb_b > 6 ~ 3",
                 "wildlife_habitat_areas <= 2 ~ 1, wildlife_habitat_areas <= 10 ~ 2, wildlife_habitat_areas > 10 ~ 3",
                 "wildlife_habitat_areas_hectares <= 100000 ~ 1, wildlife_habitat_areas_hectares <= 300000 ~ 2, wildlife_habitat_areas_hectares > 300000 ~ 3"
  )
)

openxlsx::writeData(my_wb, "Binning Levels", bin_levels_df)
