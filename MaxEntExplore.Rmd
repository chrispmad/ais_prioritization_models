...
# ggplot() +
#   geom_sf(data = bc_alb, color = "lightgrey", alpha = 0.5)+
#   geom_raster(data = as.data.frame(raster_occ_masked, xy = TRUE),
...
# 
# ggplot(data = samp)+
#   geom_sf(data = st_boundary(bc), fill = "transparent")+
#   stat_density2d(geom = "polygon", aes(x =x, y= y,fill = after_stat(level)), bins = 4)+
...
d_alb_kde<-MASS::kde2d(samp$x, samp$y,n=1000, lims = c(x1,x2,y1,y2))
#image(d_alb_kde)
...
#plot(croprast)
z1<-croprast$lyr.1*nrow(samp) # actual abundances for the raster
